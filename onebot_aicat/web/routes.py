...
async def _list_sites(request: web.Request):
    sites = await run_sync(aiconfig.list_sites)
    return web.json_response({"success": True, **sites})
...
async def _save_site(request: web.Request):
    ...
    if "api_key" in body:
        ...
        if ak is None:
            ...
            p["api_key"] = ak.strip()
    result = await run_sync(aiconfig.save_site, p)
    return web.json_response({"success": True, **result})


async def _apply_site(request: web.Request):
    body = await _json(request)
    res = await run_sync(aiconfig.apply_site, str(body.get("id") or ""))
    if res is None:
...
async def _delete_site(request: web.Request):
    body = await _json(request)
    result = await run_sync(aiconfig.delete_site, str(body.get("id") or ""))
    return web.json_response({"success": True, **result})
...
async def _check_models(request: web.Request):
    ...
    if not models:
        return web.json_response(
            {"success": False, "error": "没有可探测的模型", "status": {}}
        )
    if not aiconfig.is_configured():
        return web.json_response(
            {"success": False, "error": "未配置 api_key", "status": {}}
        )
