...
def add_command(args: dict) -> dict:
    ...
    _commands[command_id] = {
        ...
        'api_method': 'POST' if str(args.get('api_method') or '').upper() == 'POST' else 'GET',
...
def toggle_command(command_id: str, enabled: bool) -> dict:
    ...
    _save()
    return {'ok': True, 'result': f"指令 '{command_id}' 已{'启用' if enabled else '禁用'}"}
...
def _format_object(obj, fields: list) -> str:
    ...
    entries = [f'{k}: {v}' for k, v in obj.items()
               if v is not None and not isinstance(v, (dict, list))]
    if not entries:
        nested = [f'【{k}】\n{_format_value(v, [])}' for k, v in obj.items()
...
async def match_and_execute(content: str, user_id, group_id, nickname: str) -> str | None:
    ...
    for cmd in _commands.values():
        ...
        return (response.replace('{user_id}', str(user_id))
