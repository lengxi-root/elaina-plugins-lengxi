...
async def send_pb(args: dict, meta: dict) -> dict:
    ...
    if (mode in ('elem', 'long') and is_group and not is_owner
            and group_id and str(target_id) != str(group_id)):
        return {'ok': False, 'error': '不允许跨群操作'}
    ...
    try:
        if mode == 'raw':
            ...
            res = await packet.send_elem(caller, str(target_id), bool(is_group), content)
        elif mode == 'long':
            if not target_id:
                return {'ok': False, 'error': '无法确定发送目标 target_id'}
            res = await packet.send_long(caller, str(target_id), bool(is_group), content)
        else:
...
OWNER_ONLY_TOOLS = frozenset({
    ...
    'add_scheduled_task', 'remove_scheduled_task', 'toggle_scheduled_task',
...
async def query_history_messages(args: dict, meta: dict) -> dict:
    ...
    records = await msglog.query_messages(
        meta, group_id=group_id, user_id=user_id, keyword=args.get('keyword'),
        limit=limit, offset=offset, start_time=start_time)
    if not records:
        return {'ok': True, 'result': [], 'count': 0, 'message': '没有找到符合条件的消息记录'}
    return {'ok': True, 'result': [msglog.format_row(m) for m in records], 'count': len(records)}
...
async def get_message_by_id(args: dict, meta: dict) -> dict:
    ...
    if not meta.get('is_owner'):
        ...
        elif str(msg.get('user_id') or '') != str(meta.get('user_id')) or msg.get('group_id'):
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {
                    'data': {'type': 'string', 'description': '待解析的 pb 数据 (base64 或 hex)'},
                    'input_format': {'type': 'string', 'enum': ['auto', 'base64', 'hex'],
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {
                    'content': {'type': 'object', 'description': '字段号->值 的 pb 结构'},
                    'mode': {'type': 'string', 'enum': ['elem', 'long', 'raw'],
                             'description': '发送方式, 默认: 有 cmd 时 raw, 否则 elem'},
                    'cmd': {'type': 'string', 'description': 'raw 模式的协议命令, 如 MessageSvc.PbSendMsg'},
                    'target_id': {'type': 'string', 'description': 'elem/long 目标 (群号或QQ), 默认当前会话'},
                    'is_group': {'type': 'boolean', 'description': '目标是否群聊, 默认按当前会话判断'},
                },
                'required': ['content'],
            },
        },
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'engine': {'type': 'string', 'enum': ['auto', 'baidu', 'bing'],
        ...
        'function': {
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'task_type': {'type': 'string', 'enum': ['send_message', 'api_call'],
                                  'description': '任务类型'},
                    'target_type': {'type': 'string', 'enum': ['group', 'private'],
                                    'description': '目标类型'},
                    'target_id': {'type': 'string', 'description': '目标ID (群号或QQ号)'},
                    'content': {'type': 'string', 'description': '消息内容 (支持CQ码) 或 API地址'},
            ...
            'parameters': {
                ...
                'properties': {
                    ...
                    'action_content': {'type': 'string',
                                       'description': ('操作内容: reply时为回复文本, ban时为禁言秒数, '
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {'watcher_id': {'type': 'string', 'description': '检测器ID'}},
                'required': ['watcher_id'],
            },
        },
    },
    ...
    },
    {
        ...
        'function': {
            ...
            'parameters': {
                'type': 'object',
                'properties': {'command_id': {'type': 'string', 'description': '指令ID'}},
                'required': ['command_id'],
            },
        },
    },
...
async def run_tool(name: str, args: dict, meta: dict) -> dict:
    ...
    if name == 'remove_scheduled_task':
        return tasks.remove_task(str(args.get('task_id') or ''))
    if name == 'list_scheduled_tasks':
        return tasks.list_tasks()
    if name == 'toggle_scheduled_task':
        return tasks.toggle_task(str(args.get('task_id') or ''), bool(args.get('enabled')))
    if name == 'run_scheduled_task_now':
        return await tasks.run_task_now(str(args.get('task_id') or ''))
    if name == 'add_user_watcher':
        return watchers.add_watcher(args)
    if name == 'remove_user_watcher':
        return watchers.remove_watcher(str(args.get('watcher_id') or ''))
    if name == 'list_user_watchers':
        return watchers.list_watchers()
    if name == 'toggle_user_watcher':
        return watchers.toggle_watcher(str(args.get('watcher_id') or ''), bool(args.get('enabled')))
    if name == 'add_custom_command':
        return customcmd.add_command(args)
    if name == 'remove_custom_command':
        return customcmd.remove_command(str(args.get('command_id') or ''))
    if name == 'list_custom_commands':
        return customcmd.list_commands()
    if name == 'toggle_custom_command':
        return customcmd.toggle_command(str(args.get('command_id') or ''), bool(args.get('enabled')))
