...
def add_watcher(args: dict) -> dict:
    ...
    _save()
    return {'ok': True, 'result': f"用户检测器 '{watcher_id}' 已添加, 监控用户 {target or '全部'}"}
...
def toggle_watcher(watcher_id: str, enabled: bool) -> dict:
    ...
    _save()
    return {'ok': True, 'result': f"检测器 '{watcher_id}' 已{'启用' if enabled else '禁用'}"}
...
def _substitute(template: str, user_id: str, group_id: str, content: str, message_id: str) -> str:
...
async def _execute_action(watcher: dict, user_id: str, group_id: str,
                          content: str, message_id: str) -> dict:
    action_content = _substitute(watcher.get('action_content') or '',
                                 user_id, group_id, content, message_id)
    action_type = watcher.get('action_type')
    api = get_api()
    try:
        if action_type == 'reply':
            if group_id:
                res = await api.call_api('send_group_msg', {
                ...
                res = await api.call_api('send_private_msg', {
                    'user_id': int(user_id), 'message': action_content})
            return {'ok': True, 'result': res}
        if action_type == 'recall':
            return {'ok': True, 'result': await api.call_api('delete_msg', {'message_id': int(message_id)})}
        if action_type == 'ban':
            if not group_id:
            ...
            try:
                ...
                duration = 600
            return {'ok': True, 'result': await api.call_api('set_group_ban', {
                'group_id': int(group_id), 'user_id': int(user_id), 'duration': duration})}
        if action_type == 'kick':
            if not group_id:
                return {'ok': False, 'error': '踢人操作需要在群聊中'}
            return {'ok': True, 'result': await api.call_api('set_group_kick', {
        ...
        if action_type == 'api_call':
            ...
            return {'ok': True, 'result': await api.call_api(
...
async def check_and_execute(user_id, group_id, content: str, message_id) -> dict | None:
    ...
    for watcher_id, watcher in list(_watchers.items()):
        ...
        if cooldown > 0 and last and (time.time() - last) < cooldown:
            continue
        result = await _execute_action(watcher, uid, gid, content or '', str(message_id))
        watcher['last_triggered'] = int(time.time())
        watcher['trigger_count'] = int(watcher.get('trigger_count') or 0) + 1
        _save()
        return {'watcher_id': watcher_id, 'action': watcher.get('action_type'), 'result': result}
    return None
