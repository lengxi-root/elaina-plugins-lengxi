...
async def query_messages(meta: dict, group_id=None, user_id=None, keyword=None,
                         limit: int = 20, offset: int = 0, start_time=None) -> list:
    """查询历史消息, 按时间倒序分页返回。"""
    svc = _log_service()
    if svc is None:
        return []
...
async def search_messages(meta: dict, pattern: str, group_id=None,
                          user_id=None, limit: int = 20) -> list:
...
async def get_message_stats(meta: dict, group_id=None) -> dict:
    ...
    total = await svc.query('message', f'SELECT COUNT(*) AS n FROM log{cond}', params, bot_qq=bot_qq)
    today_cnt = await svc.query(
        'message', f"SELECT COUNT(*) AS n FROM log{cond}{' AND' if where else ' WHERE'} timestamp >= ?",
        [*params, today], bot_qq=bot_qq)
    users = await svc.query(
        'message', f'SELECT COUNT(DISTINCT user_id) AS n FROM log{cond}', params, bot_qq=bot_qq)
    return {
...
async def get_message_by_id(meta: dict, message_id) -> dict | None:
    ...
    rows = await svc.query(
        'message', 'SELECT * FROM log WHERE message_id = ? ORDER BY id DESC LIMIT 1',
        [str(message_id)], bot_qq=resolve_bot_qq(meta))
