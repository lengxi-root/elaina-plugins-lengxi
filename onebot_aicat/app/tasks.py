...
def parse_message_content(content: str) -> list:
    ...
    if s.startswith('[') and s.endswith(']'):
        try:
            parsed = json.loads(s)
            if isinstance(parsed, list) and all(isinstance(i, dict) and i.get('type') for i in parsed):
                return parsed
    ...
    for m in _CQ_RE.finditer(content):
        if m.start() > last:
            segments.append({'type': 'text', 'data': {'text': content[last:m.start()]}})
    ...
    try:
        if task.get('task_type') == 'send_message':
            ...
            if task.get('target_type') == 'group':
                await get_api().call_api('send_group_msg', {
                    'group_id': int(task['target_id']), 'message': message})
            else:
                await get_api().call_api('send_private_msg', {
...
def list_tasks() -> dict:
    ...
    for task_id, task in _tasks.items():
        schedule = (f"每天 {task.get('daily_time')}" if task.get('daily_time')
