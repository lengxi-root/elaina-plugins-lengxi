...
_NAPCAT_LINE_RE = re.compile(r'^(?:-\s*)?(?P<cat>[^\[]*)\[(?P<title>[^\]]+)\]\((?P<url>[^)]+)\)(?::\s*(?P<desc>.*))?$')
...
async def _napcat_index() -> str:
    ...
    now = time.time()
    if _napcat_index_cache['text'] and now - _napcat_index_cache['time'] < _NAPCAT_CACHE_TTL:
...
async def get_napcat_api_doc(url: str, max_length: int = 6000) -> dict:
    ...
    if not _NAPCAT_DOC_URL_RE.match(url):
        return {'ok': False, 'error': 'url 需为 search_napcat_apis 返回的 https://napcat.apifox.cn/xxx.md 链接'}
