...
_WRITABLE = (
    ...
    "random_reply_chance",
    "allow_public_packet",
    "ai_pb_parse_owner_only",
    "ai_pb_send_owner_only",
)
...
def _builtin_ytea() -> dict:
    return {
        "id": _YTEA_ID,
        "name": "YTea 中转 (api.ytea.top)",
        "base_url": YTEA_BASE_URL,
        "api_key": "",
        "builtin": True,
    }
...
def _load_sites_raw() -> dict:
    ...
    try:
        if os.path.exists(_SITES_FILE):
            ...
            if isinstance(loaded, dict) and isinstance(loaded.get("sites"), list):
                data = {
                    "active_id": str(loaded.get("active_id") or ""),
                    "sites": loaded["sites"],
                }
    ...
    if ytea is None:
        ...
        if (legacy.get("api_key") or legacy.get("base_url")) and legacy.get(
            "provider"
        ) != "ytea":
            data["sites"].append(
                {
                    ...
                    "name": "自定义接口",
                    "base_url": str(
                        legacy.get("base_url") or DEFAULTS["base_url"]
                    ).rstrip("/"),
        ...
        if not data["active_id"]:
            data["active_id"] = (
                _YTEA_ID
                if legacy.get("provider", "ytea") == "ytea"
                else data["sites"][-1]["id"]
            )
    else:
...
def active_site() -> dict:
    raw = _load_sites_raw()
    return next(
        (s for s in raw["sites"] if s.get("id") == raw.get("active_id")), None
    ) or (raw["sites"][0] if raw["sites"] else _builtin_ytea())
...
def save_site(p: dict) -> dict:
    ...
    with _lock:
        ...
        item = (
            next((s for s in raw["sites"] if s.get("id") == sid), None) if sid else None
        )
        if item is None:
        ...
        item["name"] = (
            str(p.get("name") or "").strip() or item.get("name") or "未命名站点"
        )
        if not item.get("builtin") and "base_url" in p:
            item["base_url"] = (
                str(p.get("base_url") or "").strip().rstrip("/") or DEFAULTS["base_url"]
            )
        if "api_key" in p:
            ak = p["api_key"]
            if ak is None:
...
def delete_site(sid: str) -> dict:
    with _lock:
        raw = _load_sites_raw()
        raw["sites"] = [
            s for s in raw["sites"] if not (s.get("id") == sid and not s.get("builtin"))
        ]
