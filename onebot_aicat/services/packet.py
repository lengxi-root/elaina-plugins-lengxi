...
def _extract_hex_data(obj):
    if isinstance(obj, str):
        if (
            len(obj) > 10
            and len(obj) % 2 == 0
            and all(c in "0123456789abcdefABCDEF" for c in obj)
        ):
            return obj
        return None
    ...
    try:
        ...
        if isinstance(result, dict):
            try:
                ...
                retcode = 1500
            if result.get("status") == "failed" or retcode != 0:
                message = result.get("message") or result.get("wording")
                return {
                    "success": False,
...
async def _upload_long(event, target_id: str, is_group: bool, content):
    ...
    resp = await send_packet(
        event, "trpc.group.long_msg_interface.MsgService.SsoSendLongMsg", packet
    )
...
async def get_message_pb(event, group_id: str, message_id: str, real_seq=None) -> dict:
    ...
    return await send_packet(
        event, "trpc.msg.register_proxy.RegisterProxy.SsoGetGroupMsg", packet
    )
...
def build_message_nodes(
    bot_id, bot_name, real_seq, sender_qq, sender_name, pb_data, onebot_data=None
):
    nodes = []
    ...
    if _packet_mode == 1:
        if onebot_data is not None:
            ...
            nodes.append(
                _flat_node(
                    bot_id, "📋", json.dumps(onebot_data, ensure_ascii=False, indent=2)
                )
            )
        ...
        if body_data is not None:
            nodes.append(
                _nested_node(
                    ...
                    "📦 Body 数据",
                    "Body 数据",
                    json_dumps_with_bytes(body_data),
                )
            )
        nodes.append(
            _nested_node(
...
async def handle_get_by_seq(event, target_seq: str, group_id: str):
    try:
        ...
        nodes = build_message_nodes(
            _bot_id(event), "Bot", int(target_seq), sender_qq, sender_name, pb_data
        )
...
async def handle_get_reply(event, group_id: str):
    ...
    try:
        ...
        nodes = build_message_nodes(
            _bot_id(event),
            "Bot",
            int(real_seq) if real_seq else 0,
            sender_qq,
            sender_name,
            pb_data or {},
            msg_data.get("message") if isinstance(msg_data, dict) else None,
        )
        await _send_forward(event, group_id, nodes)
    except Exception as e:  # noqa: BLE001
        await event.reply(f"❌ 获取消息失败: {e}")
    ...
    try:
        ...
        if reply_id:
            target_real_seq = _extract_real_seq(
                await _call(event, "get_msg", {"message_id": int(reply_id)})
            )
        ...
        sender_qq, sender_name = extract_sender_info(pb_data)
        nodes = build_message_nodes(
            _bot_id(event), "Bot", previous_seq, sender_qq, sender_name, pb_data
        )
        await _send_forward(event, group_id, nodes)
    except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        if trimmed.startswith("{"):
            ...
            params = (
                ...
                else {k: v for k, v in parsed.items() if k != "action"}
            )
        else:
            lines = trimmed.split("\n")
...
async def handle_pb_command(event, json_str: str, group_id: str):
    try:
        ...
        await event.reply(
            "✅ 发送成功" if result.get("success") else f"❌ {result.get('error')}"
        )
    except Exception as e:  # noqa: BLE001
...
async def handle_raw_command(event, cmd: str, json_str: str):
    try:
        ...
        if result.get("success"):
            await event.reply(
                f"✅ 发送成功\n响应:\n{json_dumps_with_bytes(result.get('data'))}"[
                    :3000
                ]
            )
        else:
