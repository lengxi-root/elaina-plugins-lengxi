...
from core.plugins import register_route, unregister_route

from ..services import relay
from ..services.policy import EXTERNAL_CALLER, merge_plugin_rules
from ..services.qqbot import OfficialBotApiError
...
async def save_config(request):
    try:
        body = await json_body(request)
        saved = await store.replace_config(body, preserve_secret=True)
        runtime.membership_cache.clear()
        await relay.restart_bridge()
