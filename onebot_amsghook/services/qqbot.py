...
class OfficialBotBridge:
    ...
    def _intent_mask(self):
        ...
        return (
            mask
            or INTENT_VALUES["GROUP_AT_MESSAGE_CREATE"] | INTENT_VALUES["INTERACTION"]
        )
    ...
    async def _connect_once(self):
        ...
        async with self.session.ws_connect(
            url, heartbeat=None, receive_timeout=90
        ) as websocket:
            ...
            async for item in websocket:
                ...
                if opcode == OP_HELLO:
                    interval = int(
                        (payload.get("d") or {}).get("heartbeat_interval") or 30_000
                    )
                    if heartbeat_task:
    ...
    async def _dispatch(self, event_type, payload, event_id):
        if event_type == "READY":
            ...
            self.log(
                "info",
                f"官方机器人网关已连接: {self.nickname or self.bot_id or self.config.get('appid')}",
            )
            return
    ...
    async def _post(self, path, body):
        ...
        while True:
            ...
            if code != 0:
                self.log(
                    "error", f"官机链路[官方 API 业务失败]: POST {path}, code={code}"
                )
    ...
    async def send_group_markdown(
        self, group_openid, content, *, event_id="", msg_id="", keyboard=None
    ):
        body = {
            "msg_type": 2,
            "msg_seq": _msg_seq(),
    ...
    async def send_private_markdown(
        self, user_openid, content, *, event_id="", msg_id="", keyboard=None
    ):
        body = {
            "msg_type": 2,
            "msg_seq": _msg_seq(),
    ...
    async def upload_group_media(
        self, group_openid, source, file_type, *, is_url=False
    ):
    ...
    async def send_group_media(
        self, group_openid, file_info, content="", *, event_id="", msg_id=""
    ):
        body = {
            "msg_type": 7,
            "msg_seq": _msg_seq(),
