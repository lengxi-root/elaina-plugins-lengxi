...
from ..services.utils import call_api
...
async def _set_config(request: web.Request):
    ...
    conf = await store.replace_config(body)
...
async def _get_activity(request: web.Request):
    ...
    if group_id:
        return web.json_response(
            {"success": True, "activity": store.group_activity(group_id)}
        )
...
async def _get_presets(request: web.Request):
    return web.json_response(
        {"success": True, "presets": store.config().get("presets", [])}
    )
...
async def _save_preset(request: web.Request):
    ...
    if not isinstance(presets, list):
        return web.json_response(
            {"success": False, "error": "presets 需为数组"}, status=400
        )
    conf = store.config()
