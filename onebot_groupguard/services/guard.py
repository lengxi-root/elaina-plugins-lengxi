...
async def handle_card_lock_on_message(group_id, user_id, sender_card) -> None:
    ...
    if (sender_card or "") != locked:
        log.info(
            f'[名片锁定] {user_id}@{group_id} 名片异常(当前:"{sender_card}" 锁定:"{locked}"), 修正中'
        )
        await call_api(
            "set_group_card",
            {"group_id": int(group_id), "user_id": int(user_id), "card": locked},
        )
...
async def handle_blacklist(group_id, user_id, message_id, settings=None) -> bool:
    ...
    log.info(
        f"黑名单用户 {user_id} 在群 {group_id} 发言, 已撤回并踢出({'全局' if is_global else '群独立'})"
    )
    return True
...
async def handle_filter_keywords(
    group_id, user_id, message_id, text, settings=None
) -> bool:
    conf = store.config()
    settings = settings or store.get_group_settings(group_id)
    ...
    log.info(
        f"违禁词触发: 群 {group_id} 用户 {user_id} 触发「{matched}」, 惩罚等级 {level}"
    )

    masked = (
        ("*" * len(matched))
        if len(matched) <= 2
        else matched[0] + "*" * (len(matched) - 2) + matched[-1]
    )

    await call_api("delete_msg", {"message_id": message_id})
    if level == 1:
        await send_group_text(
            group_id, f"⚠️ {user_id} 消息已撤回，原因：触发违禁词「{masked}」"
        )
    if level >= 2:
        await call_api(
            "set_group_ban",
        ...
        await send_group_text(
            group_id,
            f"⚠️ {user_id} 消息已撤回并禁言 {ban_min} 分钟，原因：触发违禁词「{masked}」",
        )
    if level >= 3:
        await call_api(
            "set_group_kick",
        ...
        await send_group_text(
            group_id, f"⚠️ {user_id} 已被移出群聊，原因：触发违禁词「{masked}」"
        )
    if level >= 4 and not store.is_owner(user_id):
        ...
        if str(user_id) not in blacklist:
            ...
            await store.save()
        await send_group_text(
            group_id, f"⚠️ {user_id} 已被加入黑名单，原因：触发违禁词「{masked}」"
        )
    return True


async def handle_msg_type_filter(
    group_id, user_id, message_id, text, segments, settings=None
) -> bool:
    settings = settings or store.get_group_settings(group_id)
...
async def handle_spam_detect(
    ...
    settings = settings or store.get_group_settings(group_id)
    spam_on = (
        settings.get("spamDetect")
        if "spamDetect" in settings
        else conf.get("spamDetect")
    )
    if not spam_on:
        return False
    window = (
        int(
            (
                settings.get("spamWindow")
                if settings.get("spamWindow") is not None
                else conf.get("spamWindow")
            )
            or 10
        )
        * 1000
    ...
    threshold = int(
        (
            settings.get("spamThreshold")
            if settings.get("spamThreshold") is not None
            else conf.get("spamThreshold")
        )
        or 10
    )
    rt = get_runtime()
    ...
    if len(rt.spam_cache) > 1000:
        for k in [
            k for k, v in rt.spam_cache.items() if not v or now - v[-1] >= window
        ]:
            rt.spam_cache.pop(k, None)
    if len(stamps) >= threshold:
        ban_min = int(
            (
                settings.get("spamBanMinutes")
                if settings.get("spamBanMinutes") is not None
                else conf.get("spamBanMinutes")
            )
            or 5
        )
        await call_api(
        ...
        log.info(
            f"刷屏检测: 群 {group_id} 用户 {user_id} {window // 1000}s 内发送 {threshold} 条消息"
        )
        return True
    return False
...
async def handle_anti_recall(
    ...
    segments = (
        cached["segments"]
        if cached["segments"]
        else [{"type": "text", "data": {"text": cached["raw"]}}]
    )
        ...
        await call_api(
            "send_group_msg",
            {
                ...
                "message": [
                    {
                        "type": "text",
                        "data": {"text": f"🔔 防撤回 - 用户 {user_id} 撤回了消息：\n"},
                    },
                    *segments,
                ],
            },
...
async def handle_emoji_react(group_id, user_id, message_id, self_id) -> None:
    ...
    if conf.get("globalEmojiReact"):
        await call_api(
            "set_msg_emoji_like", {"message_id": message_id, "emoji_id": "76"}
        )
        return
...
async def handle_qa(group_id, user_id, text, settings=None) -> bool:
    ...
    settings = settings or store.get_group_settings(group_id)
    gid = str(group_id)
    is_group_custom = bool(
        conf.get("groups", {}).get(gid) and not conf["groups"][gid].get("useGlobal")
    )
    qa_list = (
        (settings.get("qaList") or [])
        if is_group_custom
        else (conf.get("qaList") or [])
    )
    if not qa_list:
        return False
    text = (text or "").strip()
