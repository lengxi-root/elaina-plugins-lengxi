...
def _onebot_data(response: Any, action_label: str) -> Any:
    ...
    if response.get("status") != "ok" or str(response.get("retcode", -1)) != "0":
        detail = (
            response.get("wording") or response.get("message") or "OneBot 接口调用失败"
        )
...
async def list_groups(request: web.Request) -> web.Response:
    ...
    groups.sort(
        key=lambda item: (str(item["group_name"]).casefold(), str(item["group_id"]))
    )
...
def _member_name(member: dict[str, Any]) -> str:
    return str(
        member.get("card") or member.get("nickname") or member.get("user_id") or ""
    )
...
async def compare_members(request: web.Request) -> web.Response:
    try:
        ...
        group_ids = list(
            dict.fromkeys(_identity(value, "群号") for value in raw_group_ids)
        )
        if len(group_ids) < 2:
