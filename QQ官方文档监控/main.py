...
def _load_subscriptions() -> dict[str, list[str]]:
    ...
    for appid, groups in apps.items():
        ...
        if not appid or not isinstance(groups, list):
            continue
        clean_groups = sorted({group.strip() for group in groups if isinstance(group, str) and group.strip()})
        if clean_groups:
...
def _notification_targets():
    ...
    for appid, bot in manager._bots.items():
        appid = str(appid)
        for owner_id in sorted({str(owner).strip() for owner in (bot.owner_ids or []) if str(owner).strip()}):
            targets.append((f'owner:{appid}:{owner_id}', bot.sender.send_to_user, owner_id))
        for group_id in subscriptions.get(appid, []):
            targets.append((f'group:{appid}:{group_id}', bot.sender.send_to_group, group_id))
    return targets
...
async def _notify_targets(message: str) -> bool:
    ...
    if not targets:
        log.warning('没有已启动且配置 owner_ids 或订阅群的目标，文档更新将在下次检查时重试')
        return False
    ...
    if delivery.get('notification_id') != notification_id or not isinstance(delivery.get('progress'), dict):
...
async def _run_check():
    ...
    try:
        ...
        if result.failed_urls:
            log.warning(f'本次有 {len(result.failed_urls)} 个页面抓取失败，将在下次检查时重试')
...
async def _start_monitor() -> None:
    ...
    try:
        ...
        elif 'interval_minutes' in raw_config and 'interval_minutes' in config:
            interval_minutes = float(config.get('interval_minutes', 5))
...
@handler(r'^设置开放订阅群$', name='设置开放订阅群', desc='把当前群绑定到当前机器人接收文档更新', owner_only=True, group_only=True)
async def subscribe_group(event, match) -> None:
    ...
    if already:
        ...
        await event.reply(f'已设置当前群为 QQ 官方文档订阅群\n绑定机器人 appid：{appid}')


@handler(r'^取消开放订阅群$', name='取消开放订阅群', desc='取消当前群的文档更新订阅', owner_only=True, group_only=True)
async def unsubscribe_group(event, match) -> None:
    ...
    await event.reply('已取消当前群的 QQ 官方文档订阅' if existed else '当前群没有订阅记录')
...
async def docs_monitor_status(event, match) -> None:
    ...
    if subscriptions:
        lines.append('订阅群：' + '；'.join(f'{appid}: {len(groups)} 个' for appid, groups in sorted(subscriptions.items())))
    else:
        lines.append('订阅群：暂无')
    if _last_error:
    ...
    if state.get('failure_details'):
        ...
        lines.extend(f'{url}: {detail}' for url, detail in state['failure_details'].items())
    await event.reply('\n'.join(lines))


@handler(r'^立即检查文档$', name='立即检查文档', desc='立即检查一次 QQ 官方文档更新', owner_only=True, cooldown=10)
async def check_docs_now(event, match) -> None:
    ...
    if result.initialized:
        ...
        text = f'检查完成，发现 {len(result.changes)} 个页面变更，已发送给主人和订阅群。'
    else:
