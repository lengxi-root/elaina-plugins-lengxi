...
async def char_art(event, match):
    ...
    if not text or text in (" ", "-", "|", "？", "?"):
        return await event.reply(
            f"<@{uid}> 你未输入字符内容，请在指令内输入字符。", buttons=_BTN
        )
    if len(text) > 1:
        return await event.reply(
            f"<@{uid}> \n请将字数限制在1字以内，防止刷屏和发布违规内容。你可以多次生成拼接。",
            buttons=_BTN,
        )
    ...
    try:
        ...
        if resp.status_code != 200:
            return await event.reply(
                f"<@{uid}> 网络请求失败 (状态码: {resp.status_code})", buttons=_BTN
            )
    ...
    content = (
        ...
        else (raw[0] if isinstance(raw, list) and raw else "")
    )
    if not content:
    ...
    lines = content.split("\n")
    processed = [
        line[1:-2] for i, line in enumerate(lines) if line and (i + 1) % 15 != 0
    ]
