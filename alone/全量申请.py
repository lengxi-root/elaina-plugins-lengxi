...
async def apply_full_access(event, match):
    ...
    if event.event_type == "GROUP_MESSAGE_CREATE":
        return await event.reply(
            f"<@{event.user_id}> 当前群已开启全量消息，无需再次申请"
        )
    bot_uin, bot_uid = _get_bot_uin_uid()
    if not bot_uin or not bot_uid:
        return await event.reply(
            f"<@{event.user_id}> 请先在插件配置 data/config.json 中填写 uin 和 uid"
        )
...
@handler(
    r"^全量申请\s*(\d{1,5})$", name="全量申请群号校验", desc="提示重新输入疑似错误群号"
)
...
@handler(
    r"^全量列表$", name="全量列表", desc="列出所有已开启全量消息的群", owner_only=True
)
async def list_full_access(event, match):
    ...
    lines.extend(f"> {row['group_id']}" for row in groups)
    await event.reply("\n".join(lines))
