...
def _is_bot_admin(group_id):
    ...
    rows = bot.log_service.query_data(
        ...
        "WHERE group_id = ? AND is_admin = 1 AND in_group = 1 LIMIT 1",
        (group_id,),
    )
    return bool(rows)
...
async def target_user(event, match):
    if not _is_full_access(event):
        await event.reply(
            '仅限全量群使用，<qqbot-cmd-input text="全量申请" show="点击这里授权全量群" />'
        )
        return
