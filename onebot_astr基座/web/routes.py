...
from core.plugins import PLUGIN, ensure_dir, get_logger, run_sync
...
async def handle_list(request):
    cfg, apps = await asyncio.gather(
        run_sync(_read_config),
        run_sync(_installed_apps),
    )
    return await _json(
        {
            "success": True,
...
async def handle_get_config(request):
    ...
    fields = await run_sync(_app_config_schema, app)
    return await _json({"success": True, "app": app, "fields": fields})
...
async def handle_save_config(request):
    ...
    cfg = await run_sync(_read_config)
    cfg.setdefault("apps", {})[app] = values
    try:
        await run_sync(_write_config, cfg)
    except Exception as e:
...
async def handle_get_mirror(request):
    mirror = await run_sync(_load_mirror_pref)
    return await _json(
        {
            "success": True,
            "mirror": mirror,
...
async def handle_save_mirror(request):
    ...
    await run_sync(_save_mirror_pref, mirror)
    label = next(
...
async def handle_install(request):
    ...
    try:
        final_name, count = await run_sync(
            _install_to_apps,
            content,
            name or "astrbot_plugin",
        )
...
async def handle_uninstall(request):
    ...
    try:
        await run_sync(_force_rmtree, target)
    except Exception as e:
...
def teardown():
    ...
    from core.plugins import unregister_page, unregister_route
