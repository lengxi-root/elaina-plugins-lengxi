...
class CommandSpec:
    ...
    def from_entry(cls, entry: dict, method_name: str, func) -> CommandSpec:
        ...
        if entry["kind"] == "regex":
            return cls(
                "regex", [method_name], method_name, {}, entry["pattern"], perm=perm
            )
...
def _collect_commands(cls) -> list:
    ...
    for klass in cls.__mro__:
        for attr_name, member in vars(klass).items():
            ...
            seen.add(attr_name)
            cmds.extend(
                CommandSpec.from_entry(entry, attr_name, member) for entry in entries
            )
    return cmds
...
def collect_app_specs(app_name: str, module_name: str, app_dir: str) -> int:
    ...
    for cls in state.STAR_SUBCLASSES:
        ...
        found += 1
        log.info(
            f"[astrbot基座] 发现插件 [{spec.name}] v{spec.version}, 指令 {len(spec.commands)} 条"
        )
    return found
...
async def _invoke(method, astr_event, event, kwargs):
    try:
        ...
        log.error(
            f"[astrbot基座] 指令 [{astr_event.message_str[:20]}] 执行异常: {e}",
            exc_info=True,
        )
...
async def instantiate_all():
    ...
    for spec in state.PLUGIN_SPECS:
        try:
            ...
            log.error(
                f"[astrbot基座] 实例化插件 [{spec.name}] 失败: {e}", exc_info=True
            )
    return context
