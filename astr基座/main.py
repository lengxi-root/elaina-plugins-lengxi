"""astr基座 (astrbot_base) 大型插件入口: 注入 shim -> 扫描 apps/ -> 注册指令 -> 渲染。

让 ElainaBot_v2 (QQ 官方机器人框架) 直接运行 AstrBot 框架插件:
把 AstrBot 插件放到 ``apps/<插件名>/`` 下 (含 ``main.py``), 或通过 Web 面板
从 GitHub 仓库 / zip 直链 / 上传 zip 安装。
"""
...
from .services import bridge, deps  # noqa: E402
...
_DEFAULT_CONFIG = {
    ...
    "pip_index": "auto",
    # 各 app 的配置覆盖, 形如 {"astrbot_plugin_xxx": {"key": "value"}}
    "apps": {},
    # 每个插件的指令前缀 (防止不同插件指令冲突), 形如 {"astrbot_plugin_xxx": "astr"};
...
def _ensure_data_alias(app: str, app_dir: str):
    """真实 AstrBot 把插件装在 <工作目录>/data/plugins/<名>, 部分插件用该
    相对路径读写自带的数据文件; 建符号链接保持兼容。"""
    base = os.path.join(os.getcwd(), "data", "plugins")
    link = os.path.join(base, app)
    try:
        ...
        log.debug(f"[astr基座] data/plugins/{app} 别名创建失败 (忽略): {e}")
...
def _setup():
    ...
    if not app_names:
        log.warning(f"[astr基座] apps/ 下未发现可加载的插件: {_APPS_DIR}")
    for app in app_names:
        if not bridge.is_app_enabled(app):
            log.info(f"[astr基座] app [{app}] 已停用, 跳过加载")
            continue
        app_dir = os.path.join(_APPS_DIR, app)
        _ensure_data_alias(app, app_dir)
        try:
            deps.ensure_requirements(app, app_dir)
        except Exception as e:
            log.warning(f"[astr基座] [{app}] 依赖检查异常 (继续尝试加载): {e}")
        mod_name = f"{__package__}.apps.{app}.main"
        try:
            importlib.import_module(mod_name)
            n = bridge.collect_app_specs(app, mod_name, app_dir)
            log.info(f"[astr基座] 已加载 app: {app} (插件 {n} 个)")
        except Exception as e:
            log.error(f"[astr基座] 加载 app [{app}] 失败: {e}", exc_info=True)
...
async def _astrbase_on_load():
    ...
    try:
        ...
        log.warning(f"[astr基座] 接入框架 playwright 失败 (将用插件自带渲染): {e}")
    try:
        webpanel.setup()
    except Exception as e:
        log.warning(f"[astr基座] 注册 Web 面板失败: {e}")
    log.info("[astr基座] 加载完成")


@on_unload
async def _astrbase_on_unload():
    """注销 Web 面板 + 终止插件 + 卸载兼容层。"""
    try:
        webpanel.teardown()
    except Exception as e:
        log.warning(f"[astr基座] 注销 Web 面板失败: {e}")
    try:
