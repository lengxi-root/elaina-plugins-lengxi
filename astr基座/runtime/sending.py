...
async def _prepare_image(sender, image):
    ...
    if isinstance(image, str) and not image.startswith(("http://", "https://")):
        try:
            ...
            log.warning(f"[astr基座] 读取本地图片失败: {e}")
            data = None
    w = h = 0
    if data is not None:
        try:
...
async def _host_image(img_bytes: bytes):
    ...
    if not hosting:
        log.warning("[astr基座] 跳过 markdown 发图: 框架未加载「图床服务」模块")
        return None
...
async def _send_text(sender, *, event=None, group_id=None, user_id=None, content, buttons=None):
    ...
    try:
        if event is not None:
            res = await sender.reply(event, content, buttons=buttons)
        elif group_id:
            res = await sender.send_to_group(group_id, content, buttons=buttons)
        elif user_id:
            res = await sender.send_to_user(user_id, content, buttons=buttons)
        else:
            return False
        return res if isinstance(res, dict) else True
    except Exception as e:
        log.warning(f"[astr基座] 文本发送失败: {e}")
        return False
...
async def _send_one_image(sender, *, event=None, group_id=None, user_id=None, image, content=""):
    ...
    if len(caption) > _CAPTION_MAX:
        log.info(f"[astr基座] 图片说明过长({len(caption)}字)疑似原始数据, 已省略")
        caption = ""
    ...
    try:
        if event is not None:
            res = await sender.reply_image(event, data, caption)
        elif group_id:
            res = await sender.send_image("group", group_id, data, content=caption)
        elif user_id:
            res = await sender.send_image("user", user_id, data, content=caption)
        else:
            return False
        return res if isinstance(res, dict) else True
    except Exception as e:
        log.warning(f"[astr基座] 原生图片发送失败: {e}")
        return False
...
def _flatten_nodes(items) -> list:
    """展平合并转发 Nodes/Node (官机无转发能力, 逐段直发)。"""
    out: list = []
    for c in items:
        if isinstance(c, Nodes):
            out.extend(_flatten_nodes(c.nodes or []))
        elif isinstance(c, Node):
            out.extend(_flatten_nodes(c.content or []))
            out.append(Plain("\n"))
        else:
            out.append(c)
    return out
