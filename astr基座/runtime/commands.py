...
class _EnumFallbackMeta(type):
    """未定义的枚举成员返回成员名字符串, 避免 AttributeError。"""
...
class PlatformAdapterType(metaclass=_EnumFallbackMeta):
    AIOCQHTTP = "aiocqhttp"
...
def _group_decorator(name: str):
    """@filter.command_group(name): 返回装饰器, 产出 CommandGroup。"""
...
class _FilterModule(types.ModuleType):
    ...
    def __init__(self, name="astrbot.api.event.filter"):
        ...
        self.PlatformAdapterType = PlatformAdapterType
    ...
    def platform_adapter_type(self, *_a, **_k):
        # 平台过滤器: 本基座只有一个平台, 直接忽略 (不阻断装饰链)
        return _passthrough_decorator()

    def on_decorating_result(self, *a, **kw):
        return _decorating_result_decorator(*a, **kw)

    def __getattr__(self, item):
        if item.startswith("__"):
            raise AttributeError(item)
        log.debug(f"[astr基座] 未适配的 filter.{item}, 该装饰器将被忽略")
        return _passthrough_decorator
...
# 全局集合: 所有已注册的指令名 (build_handlers 时填充, 用于前缀冲突检测)
...
class PluginSpec:
    def __init__(self, cls, meta: dict):
        ...
        self.hooks: list[str] = []  # on_decorating_result 钩子方法名
        self.instance = None
    ...
    for klass in cls.__mro__:
        for attr_name, member in vars(klass).items():
            ...
            if not entries:
                continue
            seen.add(attr_name)
            # 复刻 AstrBot 的过滤器交集语义: 同方法上 event_message_type 与
            # command/regex 共存时, 前者仅作场景约束, 不单独注册全量监听
            listens = [e for e in entries if e["kind"] == "listen"]
...
def _collect_hooks(cls) -> list[str]:
    """沿 MRO 扫描 @filter.on_decorating_result 标记的钩子方法名。"""
    hooks: list[str] = []
    for klass in cls.__mro__:
        for attr_name, member in vars(klass).items():
            if attr_name in hooks:
                continue
            if getattr(member, "_astrbot_hook", "") == "on_decorating_result":
...
def _read_metadata_yaml(app_dir: str) -> dict:
    for fn in ("metadata.yaml", "metadata.yml"):
        ...
        try:
            ...
            log.warning(f"[astr基座] 读取 {fn} 失败: {e}")
    return {}
...
async def _apply_decorating_hooks(astr_event, result):
    """发送前调用各插件 @filter.on_decorating_result 钩子 (可改写结果;
    钩子期内调 event.stop_event() = 拦截本次发送)。"""
    if result is None:
        return None
    if not any(spec.hooks and spec.instance for spec in state.PLUGIN_SPECS):
        return result
    normalized = astr_event.set_result(result)
    if normalized is None:  # 无法归一化的结果类型, 不进钩子直接发
        return result
    was_stopped = astr_event.is_stopped()
    astr_event.continue_event()
    for spec in state.PLUGIN_SPECS:
        if not (spec.hooks and spec.instance):
            continue
        for mname in spec.hooks:
            method = getattr(spec.instance, mname, None)
            if method is None:
                continue
            try:
                res = method(astr_event)
    ...
    if was_stopped:
        ...
        astr_event.continue_event()
    out = None if intercepted else astr_event.get_result()
...
def make_wrapper(spec: PluginSpec, cmd: CommandSpec):
    async def _wrapper(event, match):
        ...
        if method is None:
            if instance is None and not getattr(spec, "_warned_no_instance", False):
                spec._warned_no_instance = True
                log.warning(
                    f"[astr基座] [{spec.name}] 未成功实例化 (加载日志中有报错), "
                    f"指令 [{cmd.names[0]}] 无法响应; 修复报错后请重载基座"
                )
            return
        ...
        if cmd.msg_type != "all":
            ...
            if cmd.msg_type == "group" and is_direct:
                return
...
def build_handlers():
    ...
    for spec in state.PLUGIN_SPECS:
        ...
        for cmd in spec.commands:
            ...
            total += 1
    log.info(f"[astr基座] 已注册 {total} 条指令处理器")
...
def _schema_default(meta: dict):
    """从单个 schema 条目算默认值: 有 default 用 default;
    type=object 无 default 时递归用 items 各项默认值拼 dict (复刻 AstrBot)。"""
    if "default" in meta:
        return meta["default"]
    if meta.get("type") == "object" and isinstance(meta.get("items"), dict):
        out = {}
        for k, m in meta["items"].items():
            if isinstance(m, dict):
                v = _schema_default(m)
                if v is not _MISSING:
                    out[k] = v
        return out
    # 无 default 时按类型给空值 (复刻 AstrBot: 配置项总有值)
    t = meta.get("type")
        ...
        if schema_path and os.path.isfile(schema_path):
            ...
            for key, meta in schema.items():
                if isinstance(meta, dict):
                    v = _schema_default(meta)
                    if v is not _MISSING:
                        config[key] = v
    except Exception as e:
        log.warning(f"[astr基座] 读取 [{spec.name}] 配置 schema 失败: {e}")
        ...
        try:
            ...
            if init and inspect.iscoroutinefunction(init):
                try:
                    ...
                    log.warning(f"[astr基座] [{spec.name}] initialize() 异常: {e}")
            log.info(f"[astr基座] 已实例化插件 [{spec.name}]")
        except Exception as e:
            log.error(f"[astr基座] 实例化插件 [{spec.name}] 失败: {e}", exc_info=True)
    ...
    for spec in state.PLUGIN_SPECS:
        ...
        if term and inspect.iscoroutinefunction(term):
            try:
                ...
                log.warning(f"[astr基座] [{spec.name}] terminate() 异常: {e}")
        spec.instance = None
