...
def _mode_state(mode, cfg):
    ...
    return {
        ...
        "pending_count": len(
            [target_id for target_id in targets if target_id not in sent_ids]
        ),
        "records": records,
    }
...
async def api_get_config(request):
    ...
    group_state, private_state = await asyncio.gather(
        asyncio.to_thread(_mode_state, "group", cfg),
        asyncio.to_thread(_mode_state, "private", cfg),
    )
    return _ok(
        {
            "config": cfg,
...
async def api_save_config(request):
    ...
    await asyncio.to_thread(store.save_config, mode, updates)
...
async def api_clear_records(request):
    ...
    count = await asyncio.to_thread(
        store.clear_sent_targets, push._bot_appid(bot), mode
    )
