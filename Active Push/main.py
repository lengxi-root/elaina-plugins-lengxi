...
async def cmd_broadcast(event, match):
    ...
    result = await push.broadcast(
        "group", content, source="command", progress=_progress
    )
...
async def cmd_test_push(event, match):
    ...
    if not content:
        return await event.reply("消息内容不能为空")
    _ok, msg = await push.send_to_test_target("group", content, source="command")
...
async def cmd_continue_group(event, match):
    ...
    if not content:
        return await event.reply("消息内容不能为空")
    result = await push.broadcast(
        "group", content, source="command", continue_only=True
    )
    await event.reply(result.get("message", "推送结束"))
...
@handler(
    r"^私聊推送\s+([\s\S]+)$",
    name="私聊推送",
    desc="向 data.db members 中全部用户发送主动消息",
    owner_only=True,
    priority=10,
)
...
async def cmd_continue_private(event, match):
    ...
    if not content:
        return await event.reply("消息内容不能为空")
    result = await push.broadcast(
        "private", content, source="command", continue_only=True
    )
    await event.reply(result.get("message", "推送结束"))
...
async def cmd_private_test(event, match):
    ...
    if not content:
        return await event.reply("消息内容不能为空")
    _ok, msg = await push.send_to_test_target("private", content, source="command")
    await event.reply(msg)
