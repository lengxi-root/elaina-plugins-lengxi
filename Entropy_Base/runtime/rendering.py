...
async def prewarm_browser():
    ...
    try:
        await ensure()
        log.info("[熵增基座] 已预热框架共享浏览器")
    except Exception as e:
        log.debug(f"[熵增基座] 预热浏览器失败 (不影响后续渲染): {e}")
...
def apply_render_hooks():
    ...
    for mod_name, mod in list(sys.modules.items()):
        ...
        if (
            ...
            or getattr(cls, "_entropy_hooked", False)
            or not hasattr(cls, "_screenshot")
        ):
            continue
        _hook_renderer(cls)
        cls._entropy_hooked = True
        patched += 1
    if patched:
        log.info(f"[熵增基座] 已为 {patched} 个渲染器接入框架 playwright 模块")
    return patched
    ...
    async def _screenshot_hooked(self, *args, **kwargs):
        ...
        if fw is not None:
            try:
                ...
                log.debug(f"[熵增基座] 接入框架浏览器失败, 回退插件自带: {e}")
