...
async def _generate(code: str, images: list, texts: list, args: str):
    ...
    try:
        ...
        async with (
            ...
            s.post(url, data=form, timeout=timeout) as r,
        ):
            if r.status == 200:
                return await r.read()
            text = await r.text()
            try:
                j = json.loads(text)
                err = (
                    j.get("detail") or j.get("error") or j.get("message") or "未知错误"
                )
...
async def _add_master(event, msg, user_id) -> bool:
    ...
    if qq in ids:
        ...
        return True
    ids.append(qq)
    await run_sync(config.set_owner_qqs, ids)
...
async def _remove_master(event, msg, user_id) -> bool:
    ...
    if qq == user_id and len(ids) == 1:
        ...
        return True
    await run_sync(config.set_owner_qqs, [q for q in ids if q != qq])
...
async def _meme_list(event) -> bool:
    img = await run_sync(_list_image_b64)
    if img:
...
async def _generate_meme(event, msg, target):
    try:
        ...
        if text and pt.get("max_texts", 0) == 0:
            return
        default_name = (
            at_users[0]["text"].replace("@", "").strip()
            if at_users and at_users[0].get("text")
        ...
        if user_infos is None:
            user_infos = at_users or [
                {"text": default_name, "gender": sender.get("sex", "unknown")}
            ]
        ...
        result = await _generate(
            code, buffers, texts, _build_args(code, args, user_infos)
        )
