...
def _action_data(response):
    """兼容统一 OneBot 响应和历史原生红包返回值。"""
    if not isinstance(response, dict):
        return None
    if response.get("status") == "failed":
        return {
            "ok": False,
            "err_code": int(response.get("retcode") or 1500),
            "err_msg": str(
                response.get("message") or response.get("wording") or "红包接口调用失败"
            ),
        }
    ...
    async with _STATE_LOCK:
        ...
        if success:
            ...
            stats["total_amount"] = round(
                float(stats.get("total_amount") or 0) + entry["amount"], 2
            )
            stats["today_count"] = int(stats.get("today_count") or 0) + 1
            stats["today_amount"] = round(
                float(stats.get("today_amount") or 0) + entry["amount"], 2
            )
...
async def _post_success_actions(self_id, packet, amount, elapsed_ms, settings):
    ...
    if settings["notify_owner"]:
        ...
        if target:
            ...
            sender_name = str(
                packet.get("sender_name") or packet.get("sender_id") or "未知"
            )
            text = (
                "抢红包成功\n"
...
async def initialize():
    ...
    log.info("QQ 抢红包插件已接入统一 notice.red_packet 事件，自动领取已启用")
...
async def handle_red_packet(self_id, packet):
    ...
    group_id = str(packet.get("group_id") or "")
    paused = (
        _GROUP_PAUSED_UNTIL.get((self_id, group_id), 0) > now if group_id else False
    )
    ...
    try:
        result = _action_data(
            await _call_api(
                ...
                {
                    "bill_no": bill_no,
                    "send_password_after": is_password_packet
        ...
        response_error = "" if isinstance(result, dict) else "领取接口返回格式错误"
    elapsed_ms = _event_elapsed_ms(packet, received_at)
...
@handler(
    r".*",
    name="统一红包事件",
    desc="处理所有渠道的标准 notice.red_packet 事件",
    priority=180,
    event_types=["notice.red_packet"],
    block=False,
)
async def handle_red_packet_event(event, match):
    extra = getattr(event, "extra", {})
    packet = extra.get("red_packet") if isinstance(extra, dict) else None
    if isinstance(packet, dict):
        await handle_red_packet(str(event.self_id or ""), dict(packet))
...
async def control_red_packet_legacy(event, match):
    ...
    if anti_detect:
        ...
        message = (
            f"防检测已开启，领取后暂停 {minutes} 分钟" if enabled else "防检测已关闭"
        )
        await event.reply(message)
        return

    list_command = re.fullmatch(
        r"(黑名单|白名单)\s+(群|用户)\s+(添加|删除)\s+(\d+)",
        command,
    )
    if list_command:
        list_type, target_type, action, target_id = list_command.groups()
        key = ("blacklist_" if list_type == "黑名单" else "whitelist_") + (
