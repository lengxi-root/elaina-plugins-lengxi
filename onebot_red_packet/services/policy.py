...
def normalize_settings(raw=None):
    ...
    result["pause_group_minutes"] = _bounded_int(
        result["pause_group_minutes"], 0, 0, 1440
    )
...
def rejection_reason(settings, packet, self_id, *, group_paused=False, now=None):
    ...
    if mode == "whitelist":
        ...
        if sender_id in (settings.get("blacklist_users") or ()):
            return "用户在黑名单"
        if any(
            keyword in wishing for keyword in settings.get("blacklist_keywords") or []
        ):
            return "命中黑名单关键词"
    if any(keyword in wishing for keyword in settings.get("block_keywords") or []):
        return "命中屏蔽词"
    if settings.get("skip_admin_owner") and int(packet.get("sender_role") or 0) in {
        2,
        3,
    }:
        return "跳过管理员或群主"
