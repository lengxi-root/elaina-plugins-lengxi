...
async def fetch_history_power(api, camp_id: str) -> dict:
    ...
    result: dict[str, Any] = {"ok": False, "byHero": {}}
    try:
    ...
    try:
        ...
        for item in ((res or {}).get("data") or {}).get("list") or []:
            ...
            result["byHero"][hero_id] = {
                "maxPower": _int(item.get("maxHeroFightPower")),
                "honor": str(honor),
            }
...
async def build_my_hero_view(api, camp_id: str, limit: int = SHOW_COUNT) -> dict | None:
    ...
    played = [
        h
        for h in ((res or {}).get("data") or {}).get("heroList") or []
    ...
    heroes = []
    for hero in picked:
        name, sub_name = split_hero_name(str(hero.get("name") or ""))
        level = _int(hero.get("skilledLevel"))
        heroes.append(
            {
                "name": name,
                "subName": sub_name,
                "heroType": "/".join(hero.get("heroTypes") or [])
                or str(hero.get("heroType") or ""),
                "skilledText": (SKILLED_NAME.get(level) or f"Lv.{level}")
                if level
                else "",
                "honorText": hero.get("honorText") or "",
                "imgUrl": await _hero_image(
                    hero, [hero.get("url"), hero.get("heroIcon")]
                ),
    ...
    return {
        ...
        "ownCount": _int(
            ((res or {}).get("data") or {}).get("hasData", {}).get("heroNum")
        )
        or len(played),
...
def parse_medal(text) -> dict:
    ...
    return {
        "area": matched.group(1).strip(),
        "rank": _int(matched.group(2)),
        "hero": matched.group(3).strip(),
        "text": raw,
    }
    ...
    for hero in heroes:
        ...
        try:
            res = await api.get_hero_record_details(
                role_id,
                hero.get("heroId"),
                role_name=ctx.get("role_name") or "",
...
def _group_medals(picked: list, medals: dict) -> dict:
    ...
    for hero in picked:
        ...
        if not items:
            none.append(
                {
                    "name": hero.get("name") or f"英雄{hero.get('heroId')}",
        ...
        for item in items:
            ...
            if not parsed["text"]:
                continue
            key = str(
                item.get("TitleType") if item.get("TitleType") is not None else ""
            )
...
def build_wall_view(
    name: str, picked: list, medals: dict, scanned: int, total: int
) -> dict:
    ...
    return {
        ...
        "footText": (
            f"扫了战力最高的 {scanned} / {total} 个英雄，指令后跟数字可以多扫"
...
def render_wall_text(
    ...
    if grouped["none"]:
        lines += [
            "",
            f"未上榜（{len(grouped['none'])}）："
            + "、".join(item["name"] for item in grouped["none"]),
        ]
    lines += [
        "",
        f"扫了战力最高的 {scanned} / {total} 个英雄，指令后跟数字可以多扫（最多 {MAX_SCAN}）",
    ]
    return "\n".join(lines)
