...
def _local(ts_sec: int) -> time.struct_time:
...
def week_start(now_ms: float | None = None) -> int:
    ...
    return int(time.mktime((lt.tm_year, lt.tm_mon, lt.tm_mday - back, 0, 0, 0, 0, 0, -1)))
...
def resolve_range(kind: str, now_ms: float | None = None) -> dict:
    ...
    if kind == "weekly":
        ...
        if now_sec - this_week < 2 * _DAY_SEC:
            return {"from_sec": week_start((this_week - 1) * 1000), "to_sec": this_week - 1,
                    "scope_text": "上周", "is_prev": True}
        return {"from_sec": this_week, "to_sec": now_sec, "scope_text": "本周至今", "is_prev": False}

    if kind == "monthly":
        this_month = month_start(now_ms)
        if now_sec - this_month < 2 * _DAY_SEC:
            return {"from_sec": month_start((this_month - 1) * 1000), "to_sec": this_month - 1,
                    "scope_text": "上个月", "is_prev": True}
        return {"from_sec": this_month, "to_sec": now_sec, "scope_text": "本月至今", "is_prev": False}

    return {"from_sec": today_start(now_ms), "to_sec": now_sec,
            "scope_text": "今日", "is_prev": False}
...
def group_by_day(battles: list) -> list:
    ...
    for item in battles:
        ...
        entry = days.get(key)
        if not entry:
            entry = {"date": key, "weekday": _WEEKDAY[lt.tm_wday],
                     "count": 0, "win": 0, "lose": 0, "ts": ts}
            days[key] = entry
        entry["count"] += 1
        result = _int(item.get("gameresult"))
        if result == 1:
...
def summarize_hours(hours: list, total: int) -> dict:
    ...
    return {
        "busyHour": busy_hour, "busyCount": busy_count,
...
def rank_heroes(battles: list, hero_map: dict) -> list:
    ...
    for item in battles:
        ...
        if not entry:
            entry = {"heroId": hid, "name": hero_map.get(hid) or f"英雄{hid}",
                     "count": 0, "win": 0, "lose": 0, "gradeSum": 0.0, "graded": 0}
            heroes[hid] = entry
        entry["count"] += 1
        result = _int(item.get("gameresult"))
        if result == 1:
    ...
    for entry in heroes.values():
        ...
        out.append({**entry,
                    ...
                    "avgGrade": f"{entry['gradeSum'] / entry['graded']:.1f}" if entry["graded"] else ""})
...
def summarize_session(list_: list, since: int = 0) -> dict:
    ...
    since = _int(since)
    played = [x for x in (list_ or []) if _int(x.get("dtEventTime")) >= since] if since > 0 else []
    ...
    return {
        ...
        "jobFrom": job_from, "starFrom": star_from, "jobNumFrom": job_num_from,
        "jobTo": str(last_ranked.get("roleJobName") or "").strip() if last_ranked else "",
...
def format_star_change(session: dict) -> dict | None:
    ...
    if job_from != job_to:
        return {"text": f"段位 {job_from} → {job_to}（{star_to}星）", "icon": "📈", "tone": "up"}

    if num_from and num_to and num_from != num_to:
        if num_to > num_from:
            return {"text": f"{job_to} {star_from} → {star_to}星（升段）", "icon": "📈", "tone": "up"}
        return {"text": f"{job_to} {star_from} → {star_to}星", "icon": "", "tone": "flat"}

    if star_to != star_from:
        diff = star_to - star_from
        return {"text": f"{job_to} {star_from} → {star_to}星（{'上了' + str(diff) if diff > 0 else '掉了' + str(-diff)}星）",
                "icon": "📈" if diff > 0 else "📉", "tone": "up" if diff > 0 else "down"}
...
def format_score_delta(session: dict) -> dict | None:
    ...
    return {"text": f"巅峰分 {score_from} → {score_to} ({'+' if diff > 0 else ''}{diff})",
            "icon": "📈" if diff > 0 else "📉", "tone": "up" if diff > 0 else "down"}


def summarize_report(battles: list, from_sec: int = 0, to_sec: int = 0,
                     hero_map: dict | None = None) -> dict:
    """汇总一个区间的战绩 (battles 倒序, 已按区间过滤)"""
    hero_map = hero_map or {}
    all_ = list(battles or [])
    list_ = [x for x in all_ if _int(x.get("dtEventTime")) <= to_sec] if to_sec > 0 else all_
    ...
    def one_battle(item):
        ...
        return {
            ...
            "heroName": hero_map.get(str(item.get("heroId"))) or f"英雄{item.get('heroId')}",
            "win": result == 1,
    ...
    return {
        "count": len(list_), "win": win, "lose": lose,
        ...
        "best": {
            ...
            "heroName": hero_map.get(str(best.get("heroId"))) or f"英雄{best.get('heroId')}",
        ...
        "stars": {k: session[k] for k in
                  ("jobFrom", "starFrom", "jobNumFrom", "jobTo", "jobNumTo", "starTo")},
...
def _pick_progress(report: dict) -> dict:
    ...
    return {
        ...
        "rankUpText": (f"{job_from} → {job_to}"
                       if star and star.get("tone") == "up" and job_from and job_to and job_from != job_to
                       else ""),
    }
...
def summarize_group(members: list) -> dict:
    ...
    for m in members:
        ...
        rows.append({
            ...
            "icon": m.get("icon") or "",
            "count": report["count"], "win": report["win"], "lose": report["lose"],
            "winRate": report["winRate"],
            "totalSec": report["totalSec"], "totalTimeText": report["totalTimeText"],
            "mvp": report["mvp"], "loseMvp": report["loseMvp"],
            "topHero": ({"heroId": top_hero["heroId"], "name": top_hero["name"],
                         "count": top_hero["count"]} if top_hero else None),
    ...
    for m in members:
        for h in (m.get("report") or {}).get("heroes") or []:
            ...
            if not entry:
                entry = {"heroId": hid, "name": h["name"], "count": 0, "win": 0,
                         "lose": 0, "users": 0}
    ...
    for entry in hero_totals.values():
        ...
        heroes.append({**entry,
                       "winRate": round(entry["win"] / decided_h * 100) if decided_h > 0 else 0})
    heroes.sort(key=lambda h: (-h["count"], -h["winRate"]))

    by_rate = lambda r: r["winRate"]
    return {
        "rows": rows,
        "memberCount": len(rows),
        "count": total_count, "win": total_win, "lose": total_lose,
        ...
        "topRate": (sorted((r for r in rows if r["count"] >= 3), key=by_rate, reverse=True) or [None])[0],
        "topMvp": (sorted((r for r in rows if r["mvp"] > 0),
                          key=lambda r: r["mvp"], reverse=True) or [None])[0],
        "topLoseMvp": (sorted((r for r in rows if r["loseMvp"] > 0),
                              key=lambda r: r["loseMvp"], reverse=True) or [None])[0],
        "topStreak": (sorted((r for r in rows if r["streak"].get("type") == "win" and r["streak"].get("count", 0) >= 3),
                             key=lambda r: r["streak"]["count"], reverse=True) or [None])[0],
        "topRise": (sorted((r for r in rows if (r["scoreDelta"] or 0) > 0),
                           key=lambda r: r["scoreDelta"], reverse=True) or [None])[0],
        "topDrop": (sorted((r for r in rows if (r["scoreDelta"] or 0) < 0),
                           key=lambda r: r["scoreDelta"]) or [None])[0],
...
def hero_icon_url(hero_id) -> str:
    ...
    return (f"https://game.gtimg.cn/images/yxzj/img201606/heroimg/{hid}/{hid}.jpg"
            if hid else "")
...
def build_report_view(report: dict, kind: str = "daily", from_sec: int = 0,
                      now_ms: float | None = None, to_sec: int = 0,
                      scope_text: str = "", covered_from: int = 0,
                      truncated: bool = False, role_name: str = "",
                      role_icon: str = "", hero_limit: int = 0) -> dict:
    ...
    change_lines = []
    for item in (format_star_change(report.get("stars") or {}),
                 format_score_delta(report.get("score") or {})):
        if item:
            change_lines.append({"text": f"{item['icon']} {item['text']}".strip(),
                                 "tone": item["tone"]})

    top_count = (report["heroes"][0]["count"] if report["heroes"] else 0) or 1
    heroes = []
    for idx, h in enumerate(report["heroes"][:limit]):
        heroes.append({**h, "rank": idx + 1, "icon": hero_icon_url(h["heroId"]),
    ...
    if (is_weekly or is_monthly) and by_day:
        ...
        facts.append({"key": "最勤快的一天", "val": f"{busiest['date']} 打了 {busiest['count']} 局", "tone": ""})
    if len(facts) % 2 == 1:
        facts.append({"key": "统计范围",
                      "val": scope_text or ("本月至今" if is_monthly else "本周至今" if is_weekly else "今日"),
                      "tone": ""})

    by_hour = report.get("byHour") or []
    max_hour = max(by_hour + [1])
    peak_hour = by_hour.index(max_hour) if max_hour in by_hour else 0
    hour_bars = [{"label": str(hour) if hour % 3 == 0 else "",
    ...
    if is_weekly or is_monthly:
        range_text = f"{prev_scope + ' ' if prev_scope else ''}{_md(from_sec)} - {_md(end_sec)}"
    else:
        range_text = f"{_md(from_sec)} {_weekday(from_sec)}"

    covered = (f"数据覆盖自 {_md(covered_from)}（更早的还没归档）"
               if truncated and covered_from > from_sec else "")

    return {
        "title": "战绩月报" if is_monthly else "战绩周报" if is_weekly else "战绩日报",
        "isWeekly": is_weekly, "isMonthly": is_monthly,
        ...
        "footText": covered or f"王者插件 · {'月报' if is_monthly else '周报' if is_weekly else '日报'}生成于 {_clock(now_ms)}",
    }


def build_group_view(group: dict, kind: str = "daily", from_sec: int = 0,
                     now_ms: float | None = None, to_sec: int = 0,
                     scope_text: str = "", group_name: str = "",
                     covered_from: int = 0, truncated: bool = False,
                     row_limit: int = 15, hero_limit: int = 6,
                     scanned: int = 0) -> dict:
    ...
    for idx, r in enumerate(group["rows"][:row_limit]):
        ...
        rows.append({
            **r,
            "rank": idx + 1,
            "topHeroIcon": hero_icon_url(r["topHero"]["heroId"]) if r.get("topHero") else "",
            "barWidth": max(6, round(r["count"] / top_count * 100)),
            "wrClass": rate_tone(r["winRate"], r["count"]),
            "rankClass": "gold" if idx == 0 else "silver" if idx == 1 else "bronze" if idx == 2 else "",
            "streakText": (f"{streak['count']}连{'胜' if streak['type'] == 'win' else '败'}"
                           if streak.get("count", 0) >= 3 else ""),
    ...
    award("胜率王", group.get("topRate"), lambda r: f"{r['name']} · {r['winRate']}%（{r['count']} 场）")
    award("MVP 收割机", group.get("topMvp"), lambda r: f"{r['name']} · {r['mvp']} 次")
    award("尽力局长", group.get("topLoseMvp"), lambda r: f"{r['name']} · 败方 MVP {r['loseMvp']} 次")
    award("连胜之星", group.get("topStreak"), lambda r: f"{r['name']} · {r['streak']['count']} 连胜")
    award("上分之王", group.get("topRise"), lambda r: f"{r['name']} · 巅峰分 +{r['scoreDelta']}")
    award("血亏之王", group.get("topDrop"), lambda r: f"{r['name']} · 巅峰分 {r['scoreDelta']}")
    award("升段之星", group.get("topRankUp"), lambda r: f"{r['name']} · {r['rankUpText']}")

    heroes = group.get("heroes") or []
    hero_top = (heroes[0]["count"] if heroes else 0) or 1
    hero_rows = []
    for idx, h in enumerate(heroes[:hero_limit]):
        hero_rows.append({**h, "rank": idx + 1, "icon": hero_icon_url(h["heroId"]),
    ...
    return {
        ...
        "scannedText": (f"{member_count} / {scanned} 人有对局"
                        if scanned > member_count else f"{member_count} 人上榜"),
        "count": group["count"], "win": group["win"], "lose": group["lose"],
...
_HERO_MAP_CACHE = {"map": None, "at": 0.0}


async def get_hero_name_map(api) -> dict:
    """heroId → 英雄名 (官网 herolist, ename 与营地 heroId 同一套), 缓存 6 小时"""
    if _HERO_MAP_CACHE["map"] is not None and time.time() - _HERO_MAP_CACHE["at"] < 6 * 3600:
