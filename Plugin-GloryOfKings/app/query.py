...
@handler(r'^王者(?:主页|卡片|信息)\s*(.*)$', name='王者主页', desc='查询王者营地主页/资料卡')
async def cmd_homepage(event, match):
    ...
    ok = await render.send_html(event, "MyKingHomepage.html", pdata,
                                ...
                                buttons=btns, name_hint="homepage")
    if not ok:
...
@handler(r'^(?:查询|王者)战绩(?!\s*(?:报告|导出|日\s*报|周\s*报|月\s*报))\s*(.*)$',
         name='王者战绩', desc='查询王者战绩列表 / 单局详情')
async def cmd_battle(event, match):
    ...
    if 0 < index < 9999:
        ...
        ok = await render.send_html(event, "QueryGameRecordDetails.html", detail,
    ...
    ok = await render.send_html(event, "QueryGameRecordList.html", ldata,
                                caption=caption, buttons=btns, name_hint="battlelist")
    if not ok:
