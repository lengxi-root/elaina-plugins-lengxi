...
async def _start_login(event, *, is_global: bool):
    ...
    ok = await render.send_html(
        event,
        "scanCodeLogin.html",
        {"qrCodeFile": session_info["qrcodeBase64"]},
    ...
    if not ok:
        ...
        return await event.reply(
            f"<@{event.user_id}> 生成登录二维码图片失败, 请稍后重试"
        )
...
async def _poll_qq_login(event, session, qq: str, is_global: bool):
    ...
    try:
        ...
        if is_global:
            ...
            _get_runtime().db.add_binding(
                qq, str(saved.get("userId")), role_name=str(saved.get("nickname") or "")
            )
            await event.reply(
                f"<@{qq}> QQ 登录成功\n营地: {saved.get('nickname') or saved.get('userId')} ({_mask(saved.get('userId'))})\n已绑定当前营地ID"
            )
