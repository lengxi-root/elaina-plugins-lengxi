...
async def _render_or_reply(event, template, title, data, hint):
    try:
        ok = await render.send_html(event, template, data if isinstance(data, dict) else {'data': data}, caption=f'<@{event.user_id}> {title}', name_hint=hint)
        if ok:
            return
    except Exception:
        pass
...
async def _cid(event, rt):
    ...
    if not cid: await event.reply(f"<@{event.user_id}> 请先发送 王者绑定 营地ID")
    return cid
...
def _report_text_summary(view: dict) -> str:
    ...
    lines = [f"{view['title']}（{view['rangeText']}）",
             f"总场次 {view['count']} · 胜 {view['win']} / 负 {view['lose']} · 胜率 {view['winRate']}% · 总时长 {view['totalTimeText']}"]
    if view.get("heroes"):
        tops = " · ".join(f"{h['name']} {h['count']}场({h['winRate']}%)" for h in view["heroes"][:5])
...
async def _battle_report(event, kind: str, arg: str = ""):
    ...
    if not collected["battles"]:
        return await event.reply(f"<@{event.user_id}> {_scope_word(kind, info['is_prev'])}还没有对局记录")

    hero_map = await R.get_hero_name_map(rt.api)
    report = R.summarize_report(collected["battles"], from_sec=info["from_sec"],
                                to_sec=info["to_sec"], hero_map=hero_map)
    role_name = camp
    for b in rt.db.list_bindings(str(event.user_id)):
        if str(b["camp_id"]) == str(camp) and b.get("role_name"):
            role_name = b["role_name"]
            break
    view = R.build_report_view(report, kind=kind, from_sec=info["from_sec"],
                               to_sec=info["to_sec"], now_ms=now_ms,
                               scope_text=info["scope_text"],
                               covered_from=collected["covered_from"],
                               truncated=collected["truncated"],
                               role_name=role_name)
    ok = await render.send_html(event, "BattleReport.html", view,
...
@handler(r'^王者(?:战绩)?\s*(日|周|月)报\s*(\S+)?$', name='王者战绩日报',
         desc='战绩日报/周报/月报 (周报/月报换字)', priority=1)
...
async def _group_report(event, kind: str):
    ...
    for s in subs:
        ...
        if cid and cid not in by_camp:
            by_camp[cid] = s
    total = len(by_camp)
    targets = sorted(by_camp.items(),
                     key=lambda kv: -rt.archive.archive_range(kv[0])["latest"])[:GROUP_MAX_MEMBERS]

    seconds = max(3, len(targets) * 2)
    note = (f"本群共 {total} 个账号, 只统计了最近活跃的前 {len(targets)} 个"
            if total > len(targets) else "")
    await event.reply(f"<@{event.user_id}> 正在汇总本群 {len(targets)} 个账号的{LABEL[kind]}数据, "
                      f"约需 {seconds} 秒, 请稍候...")
    ...
    view = R.build_group_view(group, kind=kind, from_sec=info["from_sec"],
                              to_sec=info["to_sec"], now_ms=now_ms,
                              scope_text=info["scope_text"],
                              covered_from=covered_from, truncated=truncated,
                              scanned=len(targets))
    view["noteText"] = " · ".join(x for x in (note, "统计范围为已开启战绩推送的本群账号") if x)
    ok = await render.send_html(event, "GroupReport.html", view,
    ...
    if not ok:
        ...
        await event.reply(f"本群{LABEL[kind]}（{view['rangeText']}）: 共 {view['count']} 场, "
                          f"胜率 {view['winRate']}%\n{rows}")


@handler(r'^王者群(日|周|月)报$', name='王者群日报', desc='群战绩日报/周报/月报', priority=1)
...
@handler(r'^王者(?:缺皮肤|还差(?:什么|哪些)皮肤|缺哪些皮肤)\s*(.*)$',
         name='王者缺皮肤', desc='反查还差哪些皮肤', priority=1)
async def cmd_skin_missing(event, match):
    ...
    if arg_camp:
        ...
        if not 1 <= arg_index <= len(binds):
            return await event.reply(f"<@{event.user_id}> 你没有第 {arg_index} 个绑定的营地ID, "
                                     "发送 王者我的ID 查看列表")
    ...
    conf = [x for x in (data.get("heroSkinConfList") or {}).values()
    ...
    if hero_name:
        ...
        if not matched["list"]:
            hint = (f"没找到「{hero_name}」, 你是想查: {'、'.join(matched['candidates'])}"
    ...
    ok = await render.send_html(event, "SkinMissing.html", view,
...
async def _rank(event, rank_type: str, is_global: bool, force: bool):
    ...
    if not camp_ids:
        hint = ("还没有人绑定营地ID, 先发送 王者绑定 营地ID 加入排名吧"
                if is_global else "本群还没有账号加入排名, 绑定后发送 王者推送 开 订阅")
        return await event.reply(f"<@{event.user_id}> {hint}")

    old = rt.rank_snapshot.read()
    fresh = bool(old["updatedAt"]) and time.time() * 1000 - old["updatedAt"] < RK.SNAPSHOT_TTL
    need_fetch = force or not fresh or not old["entries"]
    unique = list(dict.fromkeys(camp_ids))
    if need_fetch:
        await event.reply(f"<@{event.user_id}> 正在更新 {len(unique)} 个账号的数据, "
                          f"约需 {len(unique) * 2} 秒, 请稍候...")
        snapshot = await RK.collect_rank_data(
            rt.api, rt.rank_snapshot, [(c, owner_map.get(c) or "") for c in unique],
            force=True)
    else:
        snapshot = {**old, "fromCache": True}

    title = f"{'本群' if not is_global else ''}{('巅峰分' if rank_type == 'peak' else '排位')}{'总排' if is_global else '排'}行榜"
    full = RK.build_rank_list(snapshot["entries"], rank_type,
                              camp_ids=camp_ids, owner_map=owner_map)
    if not full:
        return await event.reply(
    ...
    ok = await render.send_html(event, "RankList.html", view,
                                caption=f"<@{event.user_id}> {title}",
                                name_hint="rank-list")
    if not ok:
        lines = [f"{x['index']}. {x['roleName']} "
                 + (f"巅峰分 {x['peakScore']}" if rank_type == "peak"
                    else f"{x['rankName']} {x['rankStar']}星")
                 for x in listing[:15]]
        await event.reply(f"<@{event.user_id}> {title}\n" + "\n".join(lines))


@handler(r'^王者(排位|巅峰)(总)?排名\s*(刷新)?$', name='王者排位排名',
         desc='绑定用户排位/巅峰排行榜 (总排名/刷新 可选)', priority=1)
async def cmd_rank(event, match):
    await _rank(event, "peak" if match.group(1) == "巅峰" else "rank",
                bool(match.group(2)), bool(match.group(3)))


@handler(r'^王者(?:段位榜|地区榜)$', name='王者段位榜',
         desc='绑定用户排位排行榜 (群内=本群榜, 私聊=总榜)', priority=1)
...
@handler(r'^王者(?:英雄列表|常用英雄|英雄战力榜)\s*(\S+)?$', name='王者英雄列表',
         desc='账号常用英雄榜 (赛季场次/胜率/战力)', priority=1)
async def hero_list(event, match):
    ...
    view = {
        ...
        "roleArea": " · ".join(x for x in (role.get("areaName"), role.get("roleText")) if x),
        "scopeName": picked["scopeName"],
        "showModes": picked["showModes"],
        "heroCount": len(heroes),
        "heroes": heroes,
    }
    ok = await render.send_html(event, "HeroList.html", view,
...
@handler(r'^王者(?:英雄攻略|攻略|出装|克制|铭文出装|铭文)\s*(.*)$', name='王者英雄攻略',
         desc='英雄出装/铭文/关系/技能 (官网资料库)', priority=1)
async def hero_guide(event, match):
    ...
    if not rt:
        return
    qq = str(event.user_id)
    hero_name = re.sub(r"的?(出装|攻略|克制关系)?$", "",
                       (match.group(1) or "").strip()).strip()
    if not hero_name:
        return await event.reply(
    ...
    try:
        ...
        return await event.reply(f"<@{event.user_id}> 获取「{hero_name}」的攻略失败: {e}")
    if not guide:
        return await event.reply(f"<@{event.user_id}> 没找到英雄「{hero_name}」, "
                                 "试试写全名, 如 王者英雄攻略 百里守约")
...
@handler(r'^王者(?:英雄梯度榜|英雄梯度|梯度|强度)\s*(.*)$', name='王者英雄梯度榜',
         desc='实时英雄梯度榜 (段位/分路可选)', priority=1)
async def hero_tier(event, match):
    ...
    if not ((res or {}).get("data") or {}).get("list"):
        return await event.reply(f"<@{event.user_id}> 未获取到英雄梯度榜数据, 请稍后再试")
    view = HT.build_view(res, filters["segment"], filters["position"])
    ok = await render.send_html(event, "HeroTierList.html", view,
                               caption=f"<@{event.user_id}> 英雄梯度榜 · {view['segmentLabel']} · {view['positionLabel']}",
                               name_hint="hero-tier")
    if not ok:
        lines = [f"英雄梯度榜 · {view['segmentLabel']} · {view['positionLabel']}"]
        for group in view["groups"]:
            lines.append(f"【{group['tier']}】" + "、".join(h["name"] for h in group["heroes"][:15]))
        await event.reply(f"<@{event.user_id}> " + "\n".join(lines))


@handler(r'^王者(?:英雄勋章墙|称号墙|荣耀称号|我的称号|称号列表)\s*(\S*)$',
         name='王者英雄勋章墙', desc='荣耀称号墙 (按排名列出)', priority=1)
async def medal(event, match):
    ...
    if not role_id:
        return await event.reply(f"<@{event.user_id}> 取不到角色信息, 请前往王者营地开启「陌生人可见」后重试")
    if not played:
        return await event.reply(f"<@{event.user_id}> 未获取到英雄数据, 请前往王者营地开启「陌生人可见」后重试")

    # 战力降序: 称号是战力榜, 从最高的开始扫才不会漏
    picked = sorted(played, key=lambda h: (-MH._int(h.get("heroFightPower")),
                                           -MH._int(h.get("playNum"))))[:scan]
    pending = MH.pending_medal_count(role_id, picked)
    if pending > 3:
        await event.reply(f"<@{event.user_id}> 正在逐英雄查称号（战力最高的 {len(picked)} 个）, "
    ...
    ok = await render.send_html(event, "HeroMedalWall.html", view,
...
@handler(r'^王者(?:我的英雄列表|我的英雄)\s*(.*)$', name='王者我的英雄列表',
         desc='账号全部英雄的场次/胜率/最高战力', priority=1)
async def my_heroes(event, match):
    ...
    if not view:
        return await event.reply(f"<@{event.user_id}> 未获取到英雄数据, "
                                 "请前往王者营地开启「陌生人可见」后重试")
    ok = await render.send_html(event, "MyHeroList.html", view,
    ...
    if not ok:
        ...
        for idx, hero in enumerate(view["heroes"], 1):
            lines.append(f"{idx}. {hero['name']} {hero['playNum']}场 {hero['winRate']} "
                         f"{view['powerLabel']} {hero['fightPower']}")
        await event.reply(f"<@{event.user_id}> " + "\n".join(lines))


@handler(r'^王者(?:排位趋势|分数趋势)$',name='王者排位分数趋势',desc='排位趋势分数趋势')
...
async def peak(event,match):
    ...
    except Exception as e: return await _reply(event,'巅峰赛数据不可用',str(e))
...
async def skin_news(event, match):
    ...
    ok = await render.send_html(event, "SkinNews.html", view,
                               caption=f"<@{event.user_id}> 皮肤日历 · {view['dateText']}",
                               name_hint="skin-news")
    if not ok:
        lines = [f"皮肤日历 · {view['dateText']}"]
        for label, key in (("今天上线", "todayList"), ("即将上线", "upcoming"), ("最近上线", "recent")):
            items = view[key]
            if items:
                lines.append(f"【{label}】" + "、".join(
                    f"{s['hero']}·{s['name']}（{s['countdown'] or s['dateText']}）" for s in items))
