...
from .query import _AUTH_BUTTONS, _AUTH_TIP, _need_id
...
async def cmd_fight(event, match):
    ...
    if not hero:
        return await event.reply(
            f"<@{event.user_id}> 请输入要查询的英雄名称, 如 查战力 鲁班七号"
        )
...
async def cmd_skin(event, match):
    ...
    if not hero:
        return await event.reply(
            f"<@{event.user_id}> 请输入要查询的英雄名称, 如 查皮肤 貂蝉"
        )
