...
async def cmd_push(event, match):
    ...
    if action in ("状态", ""):
        ...
        if not subs:
            return await event.reply(
                f"<@{event.user_id}> 本群暂无战绩推送订阅\n"
                "发送 <qqbot-cmd-input text='王者推送 开' /> 订阅当前账号"
            )
        lines = [
            f"{i}. {s['camp_id']}"
            + (f" {s['role_name']}" if s.get("role_name") else "")
            for i, s in enumerate(subs, 1)
        ]
        return await event.reply(
            f"<@{event.user_id}> 本群战绩推送订阅 ({len(subs)}):\n" + "\n".join(lines)
        )

    camp_id = arg if arg.isdigit() else rt.db.get_current(qq)
    if not camp_id:
        return await event.reply(
            f"<@{event.user_id}> 请先绑定营地ID, 或指定要订阅的营地ID"
        )
    ...
    if action in ("开", "on"):
        ...
        added = rt.db.add_sub(
            gid, camp_id, binds.get(camp_id, ""), subscriber=qq, appid=str(appid)
        )
        if not added:
    ...
    if action in ("关", "off"):
        sub = next(
            (
                s
                for s in rt.db.get_group_subs(gid)
                if str(s.get("camp_id")) == str(camp_id)
            ),
            None,
        )
        if not sub:
    ...
    if action in ("关", "off"):
        if arg and arg.isdigit():
            ...
            if not rt.db.remove_play_sub(gid, camp_id):
                return await event.reply(
                    f"<@{event.user_id}> 本群没有订阅营地ID {camp_id}"
                )
            return await event.reply(
                f"<@{event.user_id}> 已取消营地ID {camp_id} 的谁在游戏订阅"
            )
        removed = rt.db.remove_play_subs(gid)
        if not removed:
            return await event.reply(f"<@{event.user_id}> 本群还没有订阅谁在游戏")
        return await event.reply(
            f"<@{event.user_id}> 已取消本群 {removed} 条谁在游戏订阅"
        )
...
class PushScheduler:
    ...
    __slots__ = ("_play_task", "_rt", "_stop", "_task")
    ...
    async def _check_camp(self, camp_id: str, groups: list):
        ...
        for group_id, role_name, last_id, appid, *_ in groups:
            ...
            if not new_battles:
                ...
                continue
            ok = await self._push(
                group_id, camp_id, role_name, new_battles, battle_list, appid
            )
            if ok:
                self._rt.db.set_sub_last_battle(group_id, camp_id, newest_id)
    ...
    def _play_targets(self, gid: str, subs: list) -> dict:
        ...
        group_subs = {
            str(s.get("camp_id")): str(s.get("role_name") or "")
            for s in self._rt.db.get_group_subs(gid)
        }
        names = {
            str(b.get("camp_id")): str(b.get("role_name") or "")
        ...
        for sub in subs:
            ...
            if camp == _PLAY_ALL:
                ...
                targets.setdefault(
                    camp,
                    str(sub.get("role_name") or "")
                    or group_subs.get(camp, "")
                    or names.get(camp, ""),
                )
        if watch_all:
    ...
    async def _check_play_group(self, gid: str, subs: list):
        ...
        subscriber = next(
            (str(s.get("subscriber") or "") for s in subs if s.get("subscriber")), ""
        )
        sender = self._rt.get_sender_for(appid)
        if not sender:
            return
