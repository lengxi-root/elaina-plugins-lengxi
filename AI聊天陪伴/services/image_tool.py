...
async def _reference_bytes(url: str) -> bytes | None:
    ...
    if target.scheme not in {"http", "https"} or not await _public_host(
        target.hostname or ""
    ):
        return None
...
async def run(
    ...
    if (
        ...
        or event is None
        or service is None
        or not hasattr(service, "generate_image")
    ):
    ...
    try:
        ...
        if route_id:
            try:
                candidates = [candidates[int(route_id)]]
    ...
    except Exception:  # 媒体生成失败不应干扰用户对话或污染模型上下文。
        return {"ok": True, "sent": False}
