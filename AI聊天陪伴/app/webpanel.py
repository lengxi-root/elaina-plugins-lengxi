...
_RESOURCE_LIMITS = {'image': 20 * 1024 * 1024, 'voice': 50 * 1024 * 1024, 'video': 100 * 1024 * 1024}
...
async def _save_config(request: web.Request) -> web.Response:
    ...
    try:
        previous_files = {
            ...
            for item in config.load().get('resources', []) if item.get('file_name')
        }
        value = await asyncio.to_thread(config.save, body)
        current_files = {
            str(item.get('file_name') or '')
            for item in value.get('resources', []) if item.get('file_name')
        }
    ...
    try:
        ...
        async for part in reader:
            if part.name == 'resource_id':
                ...
                media_type = next((
                    kind for kind, extensions in _RESOURCE_MEDIA.items()
        ...
        extension = os.path.splitext(original_name)[1].casefold()
        media_type = next((kind for kind, extensions in _RESOURCE_MEDIA.items() if extension in extensions), '')
        if not chunks:
        ...
        mime_type = str(
            upload_type or mimetypes.guess_type(original_name)[0]
        ...
        for row in resources:
            if row.get('id') == resource_id:
                row.update({
                    ...
                    'original_name': original_name, 'mime_type': mime_type, 'size': size,
                })
...
async def _delete_resource_media(request: web.Request) -> web.Response:
    ...
    try:
        ...
        for row in resources:
            if row.get('id') == resource_id:
                row.update({
                    'media_type': '', 'file_name': '', 'original_name': '',
                    'mime_type': '', 'size': 0,
                })
