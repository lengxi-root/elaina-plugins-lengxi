...
def catalog_prompt(items: list[dict]) -> str:
    rows = [
        ...
        + (f'（{item.get("media_type")}媒体，读取时直接发送）' if item.get('media_type') else '')
        for item in items if item.get('enabled')
    ]
    if not rows:
        return ''
...
async def run(arguments: dict, items: list[dict], context: dict | None = None) -> dict:
    ...
    if media_type and file_path and os.path.isfile(file_path):
        ...
        return {
            'ok': True, 'sent': True, 'name': item['name'],
            'media_type': media_type, 'content': content[:12000],
        }
    if content:
