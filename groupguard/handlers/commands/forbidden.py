...
@handler(
    r"^/?违禁词添加\s+(.+)$", name="违禁词添加", desc="添加违禁词", **HANDLER_OPTIONS
)
async def cmd_fw_add(event, match):
    ...
    if not 2 <= len(word) <= 64:
        finish_action(
            event, "forbidden_add", False, details={"reason": "invalid_length"}
        )
...
async def cmd_fw_del(event, match):
    ...
    if arg.isdigit():
        ...
        if not 1 <= index <= len(words):
            finish_action(
                event, "forbidden_delete", False, details={"reason": "invalid_index"}
            )
    ...
    await reply_at(
        event, "forbidden_deleted", word=fw_render.mask_word(word), count=count
    )
...
async def cmd_fw_list(event, match):
    ...
    if result:
        finish_action(
            ...
            "forbidden_list",
            True,
            details={"count": len(words), "format": "image"},
        )
        return await reply_at(
            event, "forbidden_list_image", px=result["px"], url=result["file_url"]
        )
    masked_words = [fw_render.mask_word(word) for word in words[:300]]
    finish_action(
        event, "forbidden_list", True, details={"count": len(words), "format": "text"}
    )
    await reply_at(event, "forbidden_list_text", words=masked_words)


@handler(
    r"^/?清空违禁词\s*$", name="清空违禁词", desc="清空所有违禁词", **HANDLER_OPTIONS
)
...
async def cmd_global_fw_list(event, match):
    ...
    if not words:
        finish_action(
            event, "forbidden_list", True, details={"scope": "global", "count": 0}
        )
        return await reply_at(event, "forbidden_empty")
    finish_action(
        event, "forbidden_list", True, details={"scope": "global", "count": len(words)}
    )
    await reply_at(
        event,
        "forbidden_list_text",
...
@handler(
    r"^/?全局违禁词应用群过滤\s+(开启|开|on|关闭|关|off)$",
    name="全局违禁词应用群过滤",
    desc="设置全局违禁词是否同时用于群消息过滤",
    **HANDLER_OPTIONS,
)
async def cmd_global_fw_apply_groups(event, match):
    ...
    if not is_bot_owner(event):
        finish_action(
            event, "forbidden_global_apply", False, details={"reason": "owner_only"}
        )
        return await reply_at(event, "global_owner_required")
    if not await ensure_admin_env(event):
        return
    ...
    trace_phase(
        event,
        "forbidden_global_apply",
        "storage",
        success=True,
        details={"enabled": enabled},
    )
    finish_action(event, "forbidden_global_apply", True, details={"enabled": enabled})
    await reply_at(
        event,
        "global_forbidden_apply_changed",
