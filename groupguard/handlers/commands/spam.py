...
from ...services.responses import format_spam_punishment
...
@handler(
    r"^/?开启刷屏检测\s*$", name="开启刷屏检测", desc="开启刷屏检测", **HANDLER_OPTIONS
)
...
@handler(
    r"^/?关闭刷屏检测\s*$", name="关闭刷屏检测", desc="关闭刷屏检测", **HANDLER_OPTIONS
)
...
async def cmd_spam_limit(event, match):
    ...
    if limit < 3:
        finish_action(
            ...
            "config_change",
            False,
            details={"reason": "limit_low", "value": limit},
        )
    ...
    if limit > 100:
        finish_action(
            ...
            "config_change",
            False,
            details={"reason": "limit_high", "value": limit},
        )
...
@handler(
    r"^/?设置刷屏处罚\s*(永久|\d+)\s*$",
    name="设置刷屏处罚",
    desc="设置刷屏禁言时长（0为仅撤回，1-43200分钟为撤回并禁言）",
    **HANDLER_OPTIONS,
)
