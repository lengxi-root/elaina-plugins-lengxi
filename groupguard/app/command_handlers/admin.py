...
def _refresh_remaining(group_id, now=None):
    ...
    return max(0, int(_last_refresh.get(str(group_id), 0.0) + _REFRESH_INTERVAL - now + 0.999))
...
async def _refresh_current_group(event):
    """Refresh group metadata and bot permissions through the framework API."""
    ...
    async with lock:
        ...
        # Count failed calls too, matching the standalone refresh command.
        _remember_refresh(key)

        refresh_group_info = getattr(getattr(event, 'sender', None), 'refresh_group_info', None)
...
@handler(r'^/?群管刷新群权限\s*$', name='群管刷新群权限', desc='刷新当前群机器人权限与群资料', **HANDLER_OPTIONS)
async def cmd_refresh_group_state(event, match):
    ...
    if state is None:
        finish_action(
            event, 'refresh_group_state', False,
