...
@handler(r'^/?群管菜单\s*$', name='群管菜单', desc='查看群管控制面板', **HANDLER_OPTIONS)
...
@handler(r'^/?群管(?:分类)?\s*(用户处理|群管理|违禁词|消息过滤|刷屏检测)\s*$',
         name='群管分类', desc='查看群管分类菜单 (群管 群管理)', **HANDLER_OPTIONS)
async def cmd_category(event, match):
    begin_action(event, 'view_category', {'category': match.group(1)})
    if await ensure_admin_env(event):
        finish_action(event, 'view_category', True, details={'category': match.group(1)})
