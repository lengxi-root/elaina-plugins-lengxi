"""Persistent management statistics and audit-log queries."""
...
@handler(r'^/?群管统计(?:\s+(\d+))?\s*$', name='群管统计',
         desc='查看本群群管操作统计（默认30天）', **HANDLER_OPTIONS)
async def cmd_management_stats(event, match):
    ...
    trace_phase(event, 'view_statistics', 'storage', success=True,
                details={'days': days})
    finish_action(event, 'view_statistics', True, details={'days': days})
    await reply_at(event, 'management_stats', stats=stats)


@handler(r'^/?群管日志(?:\s+(\d+))?\s*$', name='群管日志',
         desc='查看本群最近管理操作日志（默认10条）', **HANDLER_OPTIONS)
async def cmd_management_log(event, match):
    ...
    trace_phase(event, 'view_audit_log', 'storage', success=True,
