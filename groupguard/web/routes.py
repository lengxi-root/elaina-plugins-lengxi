...
_WEB_DIR = Path(__file__).resolve().parent.parent / "assets"
_ASSETS = {
...
def _catalog_data():
    ...
    try:
        ...
        for appid, bot in bots.items():
            ...
            for row in rows:
                ...
                if (
                    not group_id
                    or not bool(row.get("in_group", 1))
            ...
            bot_item["group_count"] = sum(1 for key in groups if key[0] == appid)
            bot_items.append(bot_item)
    except Exception as error:  # noqa: BLE001
    ...
    _catalog_cache = {
        key: tuple(dict(item) for item in value) for key, value in result.items()
    }
...
def _failure(message, *, status=400, code="INVALID_REQUEST"):
...
def _developer_robot_appid(value):
    robot_appid = str(value or "").strip()
    running = {str(item.get("appid") or "") for item in _catalog_data()["bots"]}
    if not robot_appid or robot_appid not in running:
        raise ValueError("请选择正在运行的机器人")
    return robot_appid


async def _get_developer(request):
    try:
        robot_appid = _developer_robot_appid(request.query.get("appid"))
        settings = remote.public_settings(robot_appid)
...
async def _save_developer(request):
    ...
    try:
        body["robot_appid"] = _developer_robot_appid(body.get("robot_appid"))
        settings = await remote.update_settings(body)
        return _success(settings)
    except (ValueError, OSError) as error:
        return _failure(error, code="DEVELOPER_CONFIG_INVALID")
...
async def _test_developer(request):
    ...
    try:
        ...
        return _failure(error, code="BACKEND_CONNECTION_FAILED")
...
async def _get_dashboard(request):
    try:
        group_id, appid, _group = _require_managed_context(
            ...
            request.query.get("appid"),
        )
        days = _clamp_int(request.query.get("days") or 30, 1, 3650, "统计天数")
    except ValueError as error:
        return _failure(error)
    return _success(_dashboard_data(group_id, days, appid))
...
async def _save_template(request):
    ...
    try:
        ...
        return _success({"key": key, "template": template})
    except (ValueError, KeyError, OSError) as error:
        _record_web_error(
            group_id,
            "config_change",
            {"changed": ["reply_template"], "template_key": key, "error": str(error)},
            appid,
        )
        return _failure(error, code="TEMPLATE_INVALID")
...
async def _test_template(request):
    ...
    try:
        ...
        message_id = str(response.get("id") or "") if isinstance(response, dict) else ""
        return _success(
            {
                "group_id": group_id,
                "appid": appid,
...
async def _save_config(request):
    ...
    try:
        ...
        if (
            previous_spam["action"] != spam_action
        ...
        return _failure(error, code="GROUP_CONFIG_INVALID")
    ...
    try:
        ...
        updated = {
            "show_join_verification": _require_bool(
                body.get("show_join_verification"),
                "群内显示入群验证开关",
            ),
        ...
        return _failure(error, code="GLOBAL_CONFIG_INVALID")
...
async def _add_global_forbidden(request):
    ...
    try:
        ...
        if any(
            item.casefold() == word.casefold() for item in db.get_global_forbidden()
        ):
        ...
        return _success({"global_forbidden_words": _masked_global_forbidden()})
    except ValueError as error:
        _record_web_error(
            body.get("group_id"),
            "forbidden_add",
        ...
        return _failure(error, code="FORBIDDEN_WORD_INVALID")
...
async def _add_forbidden(request):
    ...
    try:
        ...
        return _success({"forbidden_words": db.get_forbidden(group_id)})
    except ValueError as error:
        _record_web_error(
            body.get("group_id"),
            "forbidden_add",
...
async def _delete_target(request):
    ...
    try:
        ...
        return _failure(error, code="TARGET_INVALID")
