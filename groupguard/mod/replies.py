...
def _framework_variables(event):
    if event is None:
        return {}
    variables = {
        name: getattr(event, name, "") or "" for name in _FRAMEWORK_EVENT_FIELDS
    }
    ...
    if isinstance(qa_list, list):
        for index, item in enumerate(qa_list, 1):
            ...
            if question or answer:
                lines.append(
                    f"问{index}：{question or empty_text}\n答{index}：{answer or empty_text}"
                )
...
def _dynamic_template(key, template, data, event=None):
    ...
    if key == "main_panel":
        ...
        category_keys = {
            "用户处理": "category_user",
            "群管理": "category_group",
            "违禁词": "category_forbidden",
            "消息过滤": "category_filter",
            "刷屏检测": "category_spam",
        }
        ...
        for index, item in enumerate(requests, 1):
            ...
            item_vars = {
                ...
                "username": redact_global_forbidden(
                    item.get("username")
                    or template.get("unknown_user_text", "未知用户")
                ),
                "avatar": (
                    f"![头像 #30px #30px]"
                    f"(https://q.qlogo.cn/qqapp/{appid}/{member_id}/640)"
                    if appid and member_id
                    else ""
                ),
                "member_id": member_id,
                "target_id": member_id,
            ...
            if (
                data.get("review_buttons", True)
                and template.get("button_mode") == "join_requests"
                and item_vars["member_id"]
                and item_vars["request_id"]
            ):
                button_rows.append(
                    _render_value(
                        button_template,
        ...
        variables.update(
            ...
            request_rows="\n".join(rows),
            next_page=_render_value(
                template.get("next_page_content", ""),
                {
                    **variables,
        ...
        if not rows:
            ...
            rendered_rows = [
                _render_value(
                    ...
                    {
                        **variables,
                        "time": datetime.fromtimestamp(item["time"]).strftime(
                            "%m-%d %H:%M:%S"
                        ),
                        "action_label": labels.get(item["action"], item["action"]),
                        "state": template.get("success_text", "成功")
                        if item["success"]
                        else template.get("failure_text", "失败"),
            ...
            _render_value(
                template.get("item_content", ""),
                {
                    **variables,
                    ...
                    "expire_at": item.get("mute_expire_at")
                    or template.get("unknown_time_text", "未知时间"),
                },
            )
        ...
        variables.update(
            ...
            member_rows="".join(member_rows),
            overflow=_render_value(
                template.get("overflow_content", ""),
                {
                    **variables,
        ...
        variables["scope_text"] = (
            template.get("scope_text", "") if data.get("user_scope") else ""
        )
        variables["failed_text"] = (
            _render_value(template.get("failed_content", ""), variables)
            if data.get("failed")
            else ""
        )
    elif key == "join_declined":
        variables["decision_text"] = (
            template.get("blacklisted_text", "已拒绝并拉黑该申请人")
...
def _build(key, data, event=None):
    template = _get_cached_template(key if key != "category_panel" else "category_spam")
