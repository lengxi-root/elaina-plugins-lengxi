    ...
    definitions = [
        (
            "skill",
            {
                ...
                "content": (
                    "新建插件时先读取 docs/plugin-development.md，以框架开发文档为首要依据；大型插件必须使用 main.py 作为唯一入口。只有文档未覆盖、"
                    "用户明确要求参考，或确实需要确认兼容行为时才读取最相关的已有插件。修改已有插件时读取"
                    "目标插件和必要的框架接口，再做局部修改。保留用户已有改动；修改后执行语法检查、相关测试"
                    "与差异检查。"
                ),
            },
        ),
